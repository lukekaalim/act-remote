...
const LinearStack = ({ orientation, children }) => {
  ...
  switch (platform.device.platformFamilyType) {
    case 'android':
      return createElement('android:linear_layout', { orientation }, children)
    case 'ios':
      return createElement('ios:stack_view', { orientation }, children)
    default:
      return null;
  }
};
const Button = ({ onClick, label }) => {
  ...
  switch (platform.device.platformFamilyType) {
    case 'android':
      return createElement('android:button', { onClick, content: label, layoutWeight: 1 })
    case 'ios':
      return createElement('ios:button', { onPress: onClick, titleText: label })
    default:
      return null;
  }
};
const Text = ({ content }) => {
  ...
  switch (platform.device.platformFamilyType) {
    case 'android':
      return createElement('android:text', { layoutWeight: 1, content })
    default:
      return null;
  }
};
...
const App = ({ platform }) => {
  ...
  return [
    createElement(bridgeContext.Provider, { value: platform }, [
      createElement(LinearStack, { orientation: 'vertical' }, [
        createElement(Text, { content: `Clicked ${count} times` }),
        createElement(Button, { onClick, label: 'Click me!' }),
      ]),
    ]),
  ];
};
